<div class="wrap">
<h2>WP Real Estate - <?php _e( 'Settings', 'wp-realestate' ); ?></h2>
<?php 
...
<form id="form1" name="form1" method="POST" action="">
  ...
  <?php endforeach; ?>
    ...
    <?php if (file_exists( ET_RE_PATH . 'pro/pro_et_er_settings.php')) { include ET_RE_PATH . 'pro/pro_et_er_settings.php'; } ?>
    <tr>
      <td><?php _e( 'Number of listing per page', 'wp-realestate' ); ?></td>
      <td><input name="et_re_pp_listing" type="text" id="et_re_pp_listing" value="<?php echo $et_re_pp_listing; ?>" /></td>
    </tr>
    <tr>
      <td><?php _e( 'Sidebar in property listing page?', 'wp-realestate' ); ?></td>
      <td><label for="p_list_sidebar"></label>
        <select name="p_list_sidebar" id="p_list_sidebar">
          <option value="1" <?php if ($p_list_sidebar == 1) {?> selected="selected"<?php } ?>><?php _e( 'Yes', 'wp-realestate' ); ?></option>
          <option value="0" <?php if ($p_list_sidebar == 0) {?> selected="selected"<?php } ?>><?php _e( 'Full Width', 'wp-realestate' ); ?></option>
        </select></td>
    </tr>
    <tr>
      <td><?php _e( 'Sidebar in property detail page?', 'wp-realestate' ); ?></td>
      <td><label for="p_detail_sidebar"></label>
        <select name="p_detail_sidebar" id="p_detail_sidebar">
          <option value="1" <?php if ($p_detail_sidebar == 1) {?> selected="selected"<?php } ?>><?php _e( 'Yes', 'wp-realestate' ); ?></option>
          <option value="0" <?php if ($p_detail_sidebar == 0) {?> selected="selected"<?php } ?>><?php _e( 'No', 'wp-realestate' ); ?></option>
        </select></td>
    </tr>
    <tr>
      <td><?php _e( 'Display Property ID (in listing)', 'wp-realestate' ); ?></td>
      <td><select name="p_pro_id_display" id="p_pro_id_display">
        <option value="1" <?php if ($p_pro_id_display == 1) {?> selected="selected"<?php } ?>><?php _e( 'Yes', 'wp-realestate' ); ?></option>
        <option value="0" <?php if ($p_pro_id_display == 0) {?> selected="selected"<?php } ?>><?php _e( 'No', 'wp-realestate' ); ?></option>
      </select></td>
    </tr>
    <tr>
      <td><?php _e( 'Max Property Photos', 'wp-realestate' ); ?></td>
      <td><input name="p_num_slider_pics" type="text" id="p_num_slider_pics" value="<?php echo $p_num_slider_pics; ?>" /></td>
    </tr>
    <tr>
      <td><?php _e( 'Display social share buttons', 'wp-realestate' ); ?></td>
      <td><select name="p_share_buttons" id="p_share_buttons">
        <option value="1" <?php if ($p_share_buttons == 1) {?> selected="selected"<?php } ?>><?php _e( 'Yes', 'wp-realestate' ); ?></option>
        <option value="0" <?php if ($p_share_buttons == 0) {?> selected="selected"<?php } ?>><?php _e( 'No', 'wp-realestate' ); ?></option>
      </select></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    ...
    <tr>
      <td><?php _e( 'More options coming soon...', 'wp-realestate' ); ?></td>
    ...
    <tr>
      <td><a href="http://www.etechysolutions.com.my/wordpress-real-estate-plugin/" target="_blank"><?php _e( 'Suggest us more features', 'wp-realestate' ); ?></a></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
...
<form id="form2" name="form2" method="post" action="">
  <table width="465" border="0" cellspacing="1" cellpadding="2">
    <tr>
      <td><h3><?php _e( 'Customize Advanced Search <em>(Mark what you want to show)', 'wp-realestate' ); ?></em></h3></td>
    </tr>
    <tr>
      <td><strong><?php _e( 'Note', 'wp-realestate' ); ?></strong>: <em><?php _e( 'Please fill the fields in property details if you have enabled the fields to be searchable below.', 'wp-realestate' ); ?></em></td>
    </tr>
    <tr>
      <td width="459"><input type="checkbox" name="adv_fld3" id="adv_fld3" disabled="disabled" /><?php _e( 'Property Title', 'wp-realestate' ); ?> <em><?php _e( '(Required)', 'wp-realestate' ); ?></em></td>
    </tr>
    <tr>
      <td>      
...
<?php _e( 'Listing Type (Sale/Rent)', 'wp-realestate' ); ?></td>
    ...
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_bedrooms" <?php if (in_array("p_bedrooms", $et_re_adv_flds)) {?> checked="checked"<?php } ?> />
       <?php _e( 'Bedrooms', 'wp-realestate' ); ?></td>
    </tr>
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_bathrooms" <?php if (in_array("p_bathrooms", $et_re_adv_flds)) {?> checked="checked"<?php } ?> />
        <?php _e( 'Bathrooms', 'wp-realestate' ); ?></td>
    </tr>
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_furnishing" <?php if (in_array("p_furnishing", $et_re_adv_flds)) {?> checked="checked"<?php } ?> /> 
        <?php _e( 'Furnishing', 'wp-realestate' ); ?>
</td>
    </tr>
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_tenure" <?php if (in_array("p_tenure", $et_re_adv_flds)) {?> checked="checked"<?php } ?> /><?php _e( 'Tenure', 'wp-realestate' ); ?> </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_cons_year" <?php if (in_array("p_cons_year", $et_re_adv_flds)) {?> checked="checked"<?php } ?> />
        <?php _e( 'Construction Year', 'wp-realestate' ); ?></td>
    </tr>
    <tr>
      <td><input name="adv_fld[]" type="checkbox" id="adv_fld[]" value="p_state" <?php if (in_array("p_state", $et_re_adv_flds)) {?> checked="checked"<?php } ?> />
        <?php _e( 'State', 'wp-realestate' ); ?></td>
    </tr>
    <tr>
      <td><input type="submit" name="button2" id="et_re_sv_search" value="<?php _e( 'Save', 'wp-realestate' ); ?>" class="button-primary" /> <img src="<?php echo admin_url( '/images/wpspin_light.gif' );?>" class="waiting" style="display:none;" id="et_re_loading2" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>
