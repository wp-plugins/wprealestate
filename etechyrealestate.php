...
function property_custom_box( $post ) {
	...
	$et_er_ref_num = stripslashes(get_post_meta($mypostid, 'et_er_ref_num', true));
...
<br style="clear:both;" />
<div class="AdmfrmLabel">
  <label for="et_er_city">Reference #</label>
</div>
<div class="AdmfrmFld">
  <input name="et_er_ref_num" type="text" id="et_er_ref_num" value="<?php echo $et_er_ref_num; ?>" />
</div>
<br style="clear:both;" />
<?php
}
...
function SavePropertyInfo($postID){
	...
	$postID = $parent_id;
		...
		if ($_POST['et_er_ref_num']) {
		update_custom_meta($postID, addslashes($_POST['et_er_ref_num']), 'et_er_ref_num');		
		}
