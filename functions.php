...
if ($wp_rewrite->permalink_structure == ''){
	...
	$frm_code .= '<form id="advncsrc" class="pfs frmshow" method="post" action="'. get_permalink($et_re_pg_pro_list).'?frm=adv">';
               ...
               $frm_code .= ' <label>'. __( "Keywords", "wp-realestate" ).'</label>
            	<input name="sbpn" placeholder="'.__( "Search Property", "wp-realestate" ).'" class="cstm_f_large" id="sbpn" value="" />
                <div class="clr"></div>';
...
$frm_code .= '<label>'. __( "Tenure", "wp-realestate" ).'</label>
                <select name="p_tenure" class="cstm_s" id="p_tenure">
                    ...
                    if ($wpcf_fields['development-tenure']['data']['options']) {
            ...
            $frm_code .= '</select>
                ...
                $frm_code .= '<label>'. __( "State", "wp-realestate" ).'</label>
...
function property_advanced_search_form2( $atts ){
	global $wpdb, $wp_query, $post, $wp_rewrite;
	$wpcf_fields = get_option('wpcf-fields');
$et_re_adv_flds = get_option('et_re_adv_flds');
...
$frm_code .= '<div class="lbel1"> <label>'. __( "Keywords", "wp-realestate" ).'</label>
            	<input name="sbpn" placeholder="'.__( "Search Property", "wp-realestate" ).'" class="cstm_f_large" id="sbpn" value="" /></div>
                <div class="clr"></div>';
             
				if($et_re_adv_flds != ""){
				if (in_array("p_list_type", $et_re_adv_flds)) {
				$frm_code .= '<div class="lbel2"><label>'. __( "Listing Type", "wp-realestate" ).'</label>
                <select name="p_list_type" class="cstm_s" id="p_list_type">
                  <option value="-1">'. __( "All", "wp-realestate" ).'</option>
                  <option value="'. __( "Sale", "wp-realestate" ).'">'. __( "For Sale", "wp-realestate" ).'</option>
                  <option value="'. __( "Rent", "wp-realestate" ).'">'. __( "For Rent", "wp-realestate" ).'</option>
              ...
              if (in_array("p_type", $et_re_adv_flds)) {
				$frm_code .= '<div class="lbel3"><label>'. __( "Property Type", "wp-realestate" ).'</label>
                <select name="p_type" class="cstm_s" id="p_type">
                	<option value="">'. __( "Any", "wp-realestate" ).'</option>';
         ...
         if (in_array("p_size", $et_re_adv_flds)) {
$frm_code .= '<div class="lbel4"><label class="labelbig">'. __( "Floor Area (Built-in)", "wp-realestate" ).'</label><br>
                <select name="p_minsize" class="cstm_s_big" id="p_minsize" style="width:60px;">
                    <option selected="selected" value="">'. __( "Min", "wp-realestate" ).'</option>
                    <option value="200">'. __( "200 sqft (18 sqm)", "wp-realestate" ).'</option>
                    <option value="500">'. __( "500 sqft (46 sqm)", "wp-realestate" ).'</option>
...
$frm_code .= '<label>'. __( "Location", "wp-realestate" ).'</label>
                ...
                if (in_array("p_price", $et_re_adv_flds)) {
$frm_code .= '<div class="lbel5"><label class="labelbig">'. __( "Price", "wp-realestate" ).'</label><br>
				<input style="width:151px !important" type="text" name="p_minprice" id="p_minprice" maxlength="7" />
                <input style="width:157px !important" type="text" name="p_maxprice" id="p_maxprice" maxlength="7" />
                </div><div class="rspbreak" style="clear:both"></div>';
				}
                if (in_array("p_bedrooms", $et_re_adv_flds)) {
$frm_code .= '<div class="lbel6"><label>'. __( "Bedrooms", "wp-realestate" ).'</label>
                <select name="p_bedrooms" class="cstm_s" id="p_bedrooms">
                    <option selected="selected" value="">'. __( "Any", "wp-realestate" ) . '</option>';
                    $bd = 0;
for ($bd = 1; $bd <= 20; $bd++) {
 ...
 $frm_code .= '</select></div>
                ...
                if (in_array("p_bathrooms", $et_re_adv_flds)) {
                $frm_code .= '<div class="lbel7"><label>'. __( "Bathrooms", "wp-realestate" ).'</label>
                <select name="p_bathrooms" class="cstm_s" id="p_bathrooms">
                    <option selected="selected" value="">Any</option>';
$bt = 0;
for ($bt = 1; $bt <= 20; $bt++) {
...
$frm_code .= ' >'. $bt.'</option>';
                ...
                if (in_array("p_furnishing", $et_re_adv_flds)) {
				$frm_code .= '<div class="lbel8"><label>'. __( "Furnishing", "wp-realestate" ).'</label>
                <select name="p_furnishing" class="cstm_s" id="p_furnishing">
                	<option selected="selected" value=""></option>
                    <option>'. __( "Not Applicable", "wp-realestate" ).'</option>
                    <option>'. __( "Unfurnished", "wp-realestate" ).'</option>
                ...
                <div class="rspbreak" style="clear:both"></div>';
				}
                if (in_array("p_cons_year", $et_re_adv_flds)) {
$frm_code .= '<div class="lbel6"><label class="labelbig">'. __( "Year Constructed", "wp-realestate" ).'</label><br>
                <select name="p_constructed_min" class="cstm_s_big" id="p_constructed_min" style="width:175px !important;margin-top: 8px;">
                    <option selected="selected" value="">'. __( "Min", "wp-realestate" ).'</option>';
                ...
                $frm_code .= '</select>
                <select name="p_constructed_max" class="cstm_s_big" id="p_constructed_max" style="width:170px !important">
                    <option selected="selected" value="">'. __( "Max", "wp-realestate" ).'</option>';
                ...
                if (in_array("p_tenure", $et_re_adv_flds)) {
$frm_code .= '<div class="lbel7"><label>'. __( "Tenure", "wp-realestate" ).'</label>
                <select name="p_tenure" class="cstm_s" id="p_tenure">
                    <option selected="selected" value="">'. __( "Any", "wp-realestate" ).'</option>
                    <option>'. __( "Freehold", "wp-realestate" ).'</option>
                    <option>'. __( "Leasehold", "wp-realestate" ).'</option>';
            ...
            $frm_code .= '</select></div>
                ...
                $frm_code .= '<input id="cstm_submit2" type="submit" value="'. __( "SEARCH", "wp-realestate" ).'" />
            <input name="adv_frm" type="hidden" id="adv_frm" value="1" />
<div style="clear:both;"></div>
            </form>
            </div>
...
add_shortcode( 'WP_RE_ADVANCED_SEARCH_2', 'property_advanced_search_form2' );
