...
if ($adv_frm == 1) {
			...
			$p_city = $_POST['p_city'];
			$et_er_ref_num = $_POST['et_er_ref_num'];
...
$query = "SELECT * FROM ".$wpdb->prefix."posts";
			...
			if($listingtype != ""){
				$query .= " INNER JOIN ".$wpdb->prefix."postmeta m1 ON ( ".$wpdb->prefix."posts.ID = m1.post_id )";
			}
			if($p_type != ""){
			...
			if($listingtype != ""){
				$query .= " AND ( m1.meta_key = 'et_er_adtype' AND m1.meta_value = '".$listingtype."' )";
			}
			if($p_type != ""){
			...
			if($p_city != ""){
				$query .= " AND ( m14.meta_key = 'et_er_city' AND m14.meta_value LIKE '%".$p_city."%')";
			}
			if($et_er_ref_num != ""){
				$query .= " AND ( m15.meta_key = 'et_er_ref_num' AND m15.meta_value = '".$et_er_ref_num."')";
			}
			if($search_sizemin != "" || $search_sizemax != ""){
...
<h2><a href="<?php echo get_permalink($propertyQuery->ID); ?>"><?php echo $propertyQuery->post_title; ?></a></h2>
      Property ID: <?php echo $propertyQuery->ID; ?><br>
      Built Up: <?php echo get_post_meta($propertyQuery->ID, 'et_er_built_size', true); ?><br>
      For <?php echo $pro_ad_type2; ?> : <?php echo ET_RE_Currency.get_post_meta($propertyQuery->ID, 'et_er_price', true); ?><br>
      Bedrooms: <?php echo get_post_meta($propertyQuery->ID, 'et_er_bedroom', true); ?><br>
      Bathrooms: <?php echo get_post_meta($propertyQuery->ID, 'et_er_bathroom', true); ?><br>
      <div style="float:left; width:100px; bottom:0px;"><a href="<?php echo get_permalink($propertyQuery->ID); ?>"><img src="<?php echo ET_RE_URL; ?>/images/view_details_button.gif" /></a></div>
  </div>
<br style="clear:both;" />
<div class="SpacerDiv"></div>
</div>
